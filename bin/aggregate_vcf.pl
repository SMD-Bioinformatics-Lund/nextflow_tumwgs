...
sub print_header {
    ...
    print
'##INFO=<ID=variant_callers,Number=.,Type=String,Description="List of variant callers which detected the variant">'
      . "\n";
    print '##FORMAT=<ID=GT,Number=1,Type=String,Description="Genotype">' . "\n";
    print '##FORMAT=<ID=DP,Number=1,Type=Integer,Description="Read Depth">'
      . "\n";
    print
'##FORMAT=<ID=VAF,Number=1,Type=Float,Description="ALT allele observation fraction">'
      . "\n";
    print
'##FORMAT=<ID=VD,Number=1,Type=Integer,Description="ALT allele observation count">'
      . "\n";
...
sub fix_gt {
    ...
    if ( $vc =~ /^(mutect2|tnscope|vardict|pindel)$/ ) {
        for my $gt ( @{ $var->{GT} } ) {
            ...
            if ( defined $fixed_gt && $fixed_gt =~ m{^1/0$} ) {
                $fixed_gt = "0/1";
            }

            add_gt( $var, $gt->{_sample_id}, "GT", $fixed_gt );
...
"\n\$ aggregate_vcf.pl --vcfs COMMA_SEP_LIST_OF_VCFS [--fluffify-pindel]\n\n";
    print
"   --vcfs        Comma separated list of VCF files to aggregate, in priority order\n";
    print "   Supported callers: " . join( ", ", @supported_callers ) . "\n\n";

    print
"   --sample-order    Comma separated list of order of samples names in output VCF.\n";
    print
"                     If not given, all input vcfs must have same order.\n\n";
    print
...
sub fluffify_pindel_variants {
    ...
    foreach my $v (@$vars) {
        if (   length( $v->{REF} ) >= $MAX_SIZE
            or length( $v->{ALT} ) >= $MAX_SIZE )
        {
...
sub vcfstr {
    ...
    for my $gt (
        sort { $order{ $a->{_sample_id} } <=> $order{ $b->{_sample_id} } }
        @{ $v->{GT} } )
    {
        my @all_gt;
        for my $key ( @{ $v->{FORMAT} } ) {
